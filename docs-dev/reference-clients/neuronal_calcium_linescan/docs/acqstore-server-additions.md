...
**Authority:** this file is the sole handoff / graft-analysis reference for this
client line. Do not use outdated sibling READMEs under
`neuronal_calcium_linescan/` for graft decisions. A separate paste-ready LLM
prompt (if needed later) should cite this document; it is not a substitute for
it.
...
and (2) if it does, produce a new output monolith that is the client HTML plus
the same class of additive AcqStore blocks. The client original is always
read-only; output is always a new file.

**Output must load as `file://`** in a browser (inline all AcqStore CSS/JS; no
`<script src>` / `<link>` to graft-kit paths).

Sources for this analysis (only):

- Client original: `client_originals/neuronal_calcium_linescan_analyzer_v1_18.html`
- Grafted output: `linescan_analyzer_v1_18_acqstore_v2_e.html`
- HTTP contract: `acqstore-server/docs/reference/api.md`

Phase B additive payload (copy source): `../graft-kit/` (see that README).
Do not paste `static/demo/v2/archive/index.monolith.html` into grafts (demo
split-pane). Do not scrape DROPIN from the v1.18 proof long-term — the kit is
the Phase B source (kit was bootstrapped once from the proof adapter).

---

## How to use (Phase A → Phase B)

### Phase A — Handoff audit (no HTML write)

**Attach:** this document + the new client monolith only.

**Produce:** PASS / PASS WITH WARNINGS / FAIL using §0 + §4.4 (+ §7 prompts).
Stop on FAIL.

### Phase B — Graft (only after PASS or PASS WITH WARNINGS)

**Attach:** this document + new client monolith + entire `graft-kit/` folder.

**Produce:** a **new** standalone HTML file =

1. Exact new client monolith bytes, plus
2. Additive inserts from the kit:
   - `reference-view.css` (inlined)
   - `load-card.html` (frozen layout)
   - `reference-host.html`
   - `reference-view.js` (inlined; disclosure card, not split-pane)
   - `load-wiring.template.js` (**adapt** setter call only)

**Hard rules:** marker-wrap all inserts; never edit `client_originals/`; never
invent a reference viewer; never add split-pane; output must open via `file://`.

Proof adapter (definition): host-safe DROPIN packaging inside
`linescan_analyzer_v1_18_acqstore_v2_e.html` — **not** the archive monolith.
The graft-kit is the extracted, self-contained form of that adapter.
...
DROPIN JS header states the viewer core is generated from the demo archive
monolith (`static/demo/v2/archive/index.monolith.html`) via a maintainer build
script. In this proof file the CSS/JS are **inlined** under the DROPIN markers.
Treat the viewer core as owned AcqStore UI: graft it additively; do not patch
client analysis to render reference images.
