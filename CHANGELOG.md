...
- Programmatic **`ServerController`** (`start` / `stop` / `status`, optional `start(reclaim=True)`) for embedders and tests; public exports (`ServerController`, `ServerStatus`, related errors) from `acqstore_server`.
- Docs: [Control the server (Python)](docs/llm/control-the-server.md) for embedders.
