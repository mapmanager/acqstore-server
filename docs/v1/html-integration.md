...
**Also read:** [`llm-agent-guide.md`](llm-agent-guide.md) (server context),
[`reference-api.md`](reference-api.md) (reference planes).
...
- `Content-Type: application/octet-stream`
- See also [`reference-api.md`](reference-api.md)
...
14. **Reference overview:** if `reference !== null`, for each `reference.channels[i]`, `GET` that channel `url`, decode float32 plane, draw overview panel(s); overlay `reference.scanPath` polyline or `reference.lineRoi` segment in the **same pixel coordinate system** as the reference plane. See [`reference-api.md`](reference-api.md). (Implemented in the in-repo fork as a collapsible card after Image Display.)
