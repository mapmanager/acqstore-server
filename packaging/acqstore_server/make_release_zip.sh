...
# Adapted from packaging/macos/make_release_zip.sh — uses RELEASE_SLUG for
# artifact names; does not require an external build-info module.
...
BUILD_INFO_JSON="{}"
if [[ -n "${BUILD_INFO_JSON_PATH:-}" && -f "$BUILD_INFO_JSON_PATH" ]]; then
...
if [[ -f "$NOTARY_SUBMISSION_ID_FILE" ]]; then
  NOTARY_ID="$(tr -d '[:space:]' < "$NOTARY_SUBMISSION_ID_FILE")"
fi

export APP_NAME BUNDLE_ID APP_VERSION BUNDLE_VERSION REL_ZIP REL_MANIFEST TEAM_ID SIGNING_AUTH NOTARY_ID
export GIT_COMMIT GIT_COMMIT_SHORT GIT_BRANCH GIT_TAG GIT_STATE BUILD_INFO_JSON RELEASE_PLATFORM
python - <<'PY'
import json
import os
import pathlib
...
manifest = {
    ...
    'release_platform': os.environ.get('RELEASE_PLATFORM', 'macos'),
